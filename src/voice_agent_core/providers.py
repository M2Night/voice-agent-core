...
from voice_agent_core.llm import build_livekit_llm, build_openrouter_llm
...
register_stt(STTProvider(name="fish", build=build_fish_stt))
register_stt(
    STTProvider(name="deepgram", build=build_deepgram_stt, models=("nova-3", "nova-2"))
)
register_tts(
    TTSProvider(name="fish", build=build_fish_tts, models=("s1", "s2-pro", "s2.1-pro"))
)
register_llm(LLMProvider(name="livekit", build=build_livekit_llm))
register_llm(LLMProvider(name="openrouter", build=build_openrouter_llm))
